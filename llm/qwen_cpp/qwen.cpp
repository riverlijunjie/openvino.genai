...
static void usage(const std::string& prog) {
    std::cout << "Usage: " << prog << " [options]\n"
        ...
        << "  --dump_exec_graph PATH  dump exec graph to path\n"
...
static Args parse_args(const std::vector<std::string>& argv) {
    ...
    for (size_t i = 1; i < argv.size(); i++) {
        ...
        else if (arg == "--dump_exec_graph") {
            args.dump_graph_path = argv[++i];
        }
        else {
...
int main(int argc, char* argv[]) try {
    ...
    if(!args.dump_graph_path.empty()) {
        ov::serialize(compilemodel.get_runtime_model(), args.dump_graph_path);
        std::cout << "Executable graph is stored to " << args.dump_graph_path << std::endl;
    }
